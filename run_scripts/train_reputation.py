# ruff: noqa: E402
"""Copy of `train.py` wired to the reputation experiment's env registry.

Differences from `train.py`:
- Imports `get_env_creator_reputation` instead of `get_env_creator` and only
  supports `--env cleanup_reputation`.
- Adds `--condition {identifiable,anonymous}` (McKee et al. 2023's core
  manipulation) plus CLI overrides for the reputation reward's
  alpha/beta sampling ranges and the contribution EMA's smoothing factor.
Everything else (algorithm config, policy specs, Ray/Tune plumbing) is
...
parser.add_argument(
    "--condition",
    type=str,
    choices=["identifiable", "anonymous"],
    default="identifiable",
    help="McKee et al. (2023) manipulation: 'identifiable' enables the reputation "
    "intrinsic reward (computed from the full group's contribution mean); "
    "'anonymous' disables the reputation reward term entirely (contributions are "
    "still tracked internally for post-hoc territoriality/turn-taking metrics).",
)
parser.add_argument(
    "--reputation_alpha_low",
    type=float,
    default=2.4,
    help="Lower bound for each agent's alpha (falling-behind-group-mean penalty weight), "
    "sampled once per agent at env construction. Paper default range: [2.4, 3.0].",
)
parser.add_argument(
    "--reputation_alpha_high",
    type=float,
    default=3.0,
    help="Upper bound for each agent's alpha. Paper default range: [2.4, 3.0].",
)
parser.add_argument(
    "--reputation_beta_low",
    type=float,
    default=0.16,
    help="Lower bound for each agent's beta (exceeding-group-mean penalty weight), "
    "sampled once per agent at env construction. Paper default range: [0.16, 0.20].",
)
parser.add_argument(
    "--reputation_beta_high",
    type=float,
    default=0.20,
    help="Upper bound for each agent's beta. Paper default range: [0.16, 0.20].",
)
parser.add_argument(
    "--contribution_ema_lambda",
    type=float,
    default=0.97,
    help="Smoothing factor for the per-agent contribution EMA (paper's lambda=0.97).",
)
parser.add_argument(
    "--contribution_scale",
    type=float,
    default=0.001,
    help="Per-step contribution increment (Eq. S3's q=1, rescaled). Default 0.001 = 1/T "
    "for T=1000-step episodes, which reconciles the paper's Eq. S4 with its Fig. S1/S2 "
    "magnitudes -- see MapEnvReputation's docstring. 1.0 = literal (implausibly large) reading.",
)
parser.add_argument(
    "--reputation_seed",
    type=int,
    default=None,
    help="If given, seeds np.random immediately before sampling each agent's alpha/beta, "
    "so the sampled reputation parameters are reproducible across runs.",
)
...
def build_experiment_config_dict(args):
    ...
    base_env_creator = get_env_creator_reputation(
        ...
        reputation_alpha_range=(args.reputation_alpha_low, args.reputation_alpha_high),
        reputation_beta_range=(args.reputation_beta_low, args.reputation_beta_high),
        contribution_ema_lambda=args.contribution_ema_lambda,
        contribution_scale=args.contribution_scale,
        reputation_seed=args.reputation_seed,
    )
...
def get_experiment_name(args):
    ...
    if sys.gettrace() is not None:
        ...
        exp_name = args.env + "_" + args.condition + "_" + args.model + "_" + args.algorithm
    else:
...
if __name__ == "__main__":
    ...
    if parsed_args.env != "cleanup_reputation":
        raise ValueError(
            "train_reputation.py only supports --env cleanup_reputation "
            f"(got '{parsed_args.env}'). Use train.py for the original envs."
        )
