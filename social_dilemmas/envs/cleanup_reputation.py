"""Cleanup environment with McKee et al. (2023)'s reputation intrinsic reward.

This is a copy of `cleanup.py` (class renamed `CleanupReputationEnv`),
subclassing `MapEnvReputation` instead of `MapEnv` so the same map/mechanics
can optionally add the reputation reward term (see
`map_env_reputation.py`'s module docstring for the mechanism).

The only functional change from `cleanup.py` is in `custom_action`'s "CLEAN"
branch: an agent's cleaning beam counts as a "contribution"
(`agent.contributed_this_step = 1`, McKee et al.'s q^t_j) only when it
actually converts at least one waste cell back to river (`updates` nonempty)
-- not merely for choosing the CLEAN action. `MapEnvReputation.step()`
already resets this flag to 0 for every agent at the top of each step, so
this only ever needs to set it to 1.
"""
...
from social_dilemmas.envs.map_env_reputation import MapEnvReputation
...
class CleanupReputationEnv(MapEnvReputation):
    def __init__(
        self,
    ...
    def custom_action(self, agent, action):
        ...
        if action == "FIRE":
            ...
            if self.reputation_reward and updates:
                # McKee et al.'s q^t_j: this agent "contributed" (cleaned at
                # least one waste cell) on this step.
                agent.contributed_this_step = 1
        return updates
